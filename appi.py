...
def check(email):
    url = "https://knglzg59si.execute-api.us-east-2.amazonaws.com/plasma/getdata?email=" + email
...
def register():
    ...
    if ('errorType' in check(x[1])):
        url = "https://knglzg59si.execute-api.us-east-2.amazonaws.com/plasma/registration?" + params
...
@app.route('/stats')
def stats():
    url = " https://knglzg59si.execute-api.us-east-2.amazonaws.com/plasma/getbloodgroupsdata"
    response = requests.get(url)
    r = response.json()
    print(r)
...
def requested():
    ...
    # print(bloodgrp)
    url = " https://knglzg59si.execute-api.us-east-2.amazonaws.com/plasma/requestonbloodgroup?blood=" + bloodgrp
    status = requests.request("GET", url)
    a = status.json()
    print(a)
    phone = []
    for i in a:
        url = "https://www.fast2sms.com/dev/bulk?authorization=gS6JZBDzKlr87awqNmFIUvAtRuxGjpCb0ey32nskY9Q4odOLihVBr0cOXw5eRCxiNjLSzPtumWd7Da4y&sender_id=FSTSMS&message=Need of plasma of your blood group&language=english&route=p&numbers=" + str(
            i['phone'])
